...
alpha=[45 70 80 89];   %%% Interesting trajectory 90, 50, 30, 10
...
for k = 1:length(alpha)
    ...
    Z =1E5;
    gamma_x = sqrt(ep0/(n0*mi))*B0*cosd(alpha(k));
    gamma_z = sqrt(ep0/(n0*mi))*B0*sind(alpha(k));
...
%     end
   ...
   Fx=((LD*B0*e0*cs)/(Te*sqrt(2))).*u;
   Fy=((LD*B0*e0*cs)/(Te*sqrt(2))).*(w-u);
   Fz=-((LD*B0*e0*cs)/(Te*sqrt(2)))*v;
...
%     v_para = sqrt(((u.*cos(alpha(k))).^2)+((w.*sin(alpha(k))).^2));
%     v_perp = sqrt(((u.*sin(alpha(k))).^2)+((w.*cos(alpha(k))).^2)+v.^2);
    v_para = sqrt((u.^2)+(w.^2));
    v_perp = sqrt(v.^2);
    ...
    plot(z(1:max_z),y((1:max_z),1),'linewidth',2); hold on
                    ...
                    legend(['\alpha = ',num2str(alpha(1)),'\circ'],['\alpha = ',num2str(alpha(2)),'\circ'],['\alpha = ',num2str(alpha(3)),'\circ'],['\alpha = ',num2str(alpha(4)),'\circ'])
...
%                     title(['B = ',num2str(B0),'T,\alpha = ',num2str(alpha(k))])
                    title(['\alpha = ',num2str(alpha(k)),'\circ'])
